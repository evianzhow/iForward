...
/**
*
int isVersion2(char *cmd)
{
  char *id = [uid UTF8String];
    char            digest[141];
  
    hmac_sha1_hex(digest, cmd, strlen(cmd), id, strlen(id));
    //NSLOG(@"HMAC-SHA1 %s = %s\n", id, digest);
  
  char *tmpSql = "select ver from version where ver='%s';";
  char *update_sql;
    update_sql = (char*) malloc((strlen(tmpSql) + strlen(digest) + 2) * sizeof(char));
    assert(update_sql != (char*) NULL);
  sprintf(update_sql, tmpSql, digest);
    //NSLOG(@"ver =%s", update_sql);
    sqlite3 * db;
    sqlite3_stmt * stmt;
    int rc;
  int rows = 0;
    ...
    while (1)
   {
    ...
    else if (s == SQLITE_DONE)
    {
        ...
        //First set the URL that is about to receive our POST. This URL can
           ...
           //data. 
        curl_easy_setopt(curl, CURLOPT_URL, "http://ifoward.com/lic.php");
        //Now specify the POST data
        char *field = (char*) malloc((strlen(id) + 6) * sizeof(char));
        assert(field != (char*) NULL);
        sprintf(field, "udid=%s", id);
        ...
        {
          NSLOG(@"update ver %s", chunk.memory);
            //ExecUpdateVerCommand("insert into version (ROWID,ver) values (1,'%s');",chunk.memory);
          free(chunk.memory);
